...
class NewCcTask extends BaseNotification
{
    ...
    public function toMail($notifiable): MailMessage
    {
        ...
        $template = EmailTemplate::where('event_name', 'NewCcTask')->where('is_active', true)->first();

        if ($template) {
            $subject = str_replace(['{task_heading}', '{task_short_code}', '{due_date}', '{notifiable_name}'], [$this->task->heading, $this->task->task_short_code, $dueDate, $notifiable->name], $template->subject);
            $content = str_replace(['{task_heading}', '{task_short_code}', '{due_date}', '{notifiable_name}', '{url}'], [$this->task->heading, $this->task->task_short_code, $dueDate, $notifiable->name, $url], $template->content);
        ...
        <p>You have been Carbon Copied (CC) on this task.</p>';

        return $build
            ->subject('CC: ' . __('email.newTask.subject') . ' #' . $this->task->task_short_code . ' - ' . config('app.name') . '.')
            ->greeting(__('email.hello') . ' ' . $notifiable->name . ',')
            ->markdown('mail.task.created', [
                'url' => $url,
                'content' => new HtmlString($content),
