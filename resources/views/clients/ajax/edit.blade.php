                ...
                <div class="row p-20">
                    ...
                    <div class="col-md-3">
                        <x-forms.select fieldId="dealer_category" fieldLabel="Dealer Category" fieldName="dealer_category" fieldRequired="true">
                            <option value="distributor" {{ ($client->clientDetails->dealer_category == 'distributor') ? 'selected' : '' }}>Distributor</option>
                            <option value="dealer" {{ ($client->clientDetails->dealer_category == 'dealer' || !$client->clientDetails->dealer_category) ? 'selected' : '' }}>Dealer</option>
                            <option value="end_customer" {{ ($client->clientDetails->dealer_category == 'end_customer') ? 'selected' : '' }}>End Customer</option>
                        </x-forms.select>
                    </div>
                    <div class="col-md-3">
                        ...
                        <x-forms.select fieldId="salesperson_id" fieldLabel="Assigned Salesperson" fieldName="salesperson_id" fieldRequired="true">
                            ...
                            @if(isset($employees))
                                @foreach($employees as $emp)
                                    <option value="{{ $emp->id }}" {{ ($client->clientDetails->salesperson_id == $emp->id) ? 'selected' : '' }}>{{ $emp->name }}</option>
                                @endforeach
                            @endif
                        </x-forms.select>
                    </div>
                ...
                <div class="row px-20 pb-20">
                    ...
                    <div class="col-md-3">
                        <x-forms.text class="mb-3 mt-3 mt-lg-0 mt-md-0" fieldId="ntn_number" fieldName="ntn_number"
                            ...
                            :fieldValue="$client->clientDetails->ntn_number ?? ''">
                        </x-forms.text>
                    </div>
                </div>
                <div class="row px-20 pb-20">
                    <div class="col-md-3">
                        <x-forms.text class="mb-3 mt-3 mt-lg-0 mt-md-0" fieldId="strn_number" fieldName="strn_number"
                            ...
                            :fieldValue="$client->clientDetails->strn_number ?? ''">
                        </x-forms.text>
                    </div>
                </div>
...
<script>
    (function() {
        ...
        $body.on('click' + namespace, '#save-form', function() {
            var url = "{{ route('clients.update', $client->id) }}";
            $.easyAjax({
                url: url,
