...
namespace App\Services\Barcode\Picqer;
...
abstract class BarcodeGenerator
{
    ...
    protected function encode_code39_ext($code)
    {
        ...
        for ($i = 0; $i < $clen; ++$i) {
            if (ord($code[$i]) > 127) {
                throw new \Exception('Only supports till char 127');
            }
    ...
    protected function barcode_msi($code, $checksum = false)
    {
        ...
        for ($i = 0; $i < $clen; ++$i) {
            $digit = $code[$i];
            if ( ! isset($chr[$digit])) {
                throw new \Exception('Char ' . $digit . ' is unsupported');
            }
            $seq .= $chr[$digit];
        }
        ...
        if ($upce) {
            ...
            for ($i = $half_len; $i < $len; ++$i) {
                if ( ! isset($codes['C'][$code[$i]])) {
                    throw new \Exception('Char ' . $code[$i] . ' not allowed');
                }
                $seq .= $codes['C'][$code[$i]];
            }
    ...
    protected function barcode_codabar($code)
    {
        ...
        for ($i = 0; $i < $len; ++$i) {
            if ( ! isset($chr[$code[$i]])) {
                throw new \Exception('Char ' . $code[$i] . ' is unsupported');
            }
            $seq = $chr[$code[$i]];
            for ($j = 0; $j < 8; ++$j) {
                if (($j % 2) == 0) {
    ...
    protected function barcode_imb($code)
    {
        ...
        switch (strlen($routing_code)) {
            ...
            default: {
                throw new \Exception('Routing code unknown');
                break;
            }
        }
