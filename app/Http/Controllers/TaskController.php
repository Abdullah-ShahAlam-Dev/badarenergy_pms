...
class TaskController extends AccountBaseController
{
        ...
        if (!is_null($request->taskId)) {
            ...
            if ($subTask) {
                foreach ($subTask as $subTasks) {
                    ...
                    // Rule 5 (STRICT): Preserve original department_id — NEVER recalculate or override
                    $subTaskData->department_id = $subTasks->department_id;
    ...
    public function show($id)
    {
        ...
        // Total sub-task count (unfiltered) for UI hidden-items indicator (Rule 8)
        $totalSubTaskCount = \App\Models\SubTask::where('task_id', $id)->count();
        ...
        $this->task = Task::with(
            ...
            // departmentScope applied here: ONLY in TaskController (Rule 9)
            'subtasks' => function ($q) use ($viewSubTaskPermission) {
                // Apply department isolation (Local Scope — not Global Scope)
                $q->departmentScope();
