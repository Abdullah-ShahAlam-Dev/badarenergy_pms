...
trait EmployeeDashboard
{
    ...
    public function employeeDashboard()
    {
        ...
        $sixMonthsAgoDay = now(company()->timezone)->subMonths(6)->format('m-d');
        $this->pastBirthdays = EmployeeDetails::whereHas('user', function ($query) {
            return $query->where('status', 'active');
        })
            ->with('user')
            ->select('*', 'date_of_birth', DB::raw('MONTH(date_of_birth) months'), DB::raw('DAY(date_of_birth) as day'))
            ->whereNotNull('date_of_birth')
            ->where(function ($query) use ($currentDay, $sixMonthsAgoDay) {
                if ($sixMonthsAgoDay > $currentDay) {
                    $query->whereRaw('DATE_FORMAT(`date_of_birth`, "%m-%d") >= "' . $sixMonthsAgoDay . '"')
                          ->orWhereRaw('DATE_FORMAT(`date_of_birth`, "%m-%d") < "' . $currentDay . '"');
                } else {
                    $query->whereRaw('DATE_FORMAT(`date_of_birth`, "%m-%d") >= "' . $sixMonthsAgoDay . '"')
            ...
            ->get()
            ->sortByDesc(function ($employee) use ($currentDay) {
                $dobDay = \Carbon\Carbon::parse($employee->date_of_birth)->format('m-d');
                return $dobDay > $currentDay ? '0' . $dobDay : '1' . $dobDay;
            })
            ->take(5)
            ->values()
            ->all();
