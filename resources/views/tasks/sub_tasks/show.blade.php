@php
    ...
    // Count of sub-tasks visible to this user (post-filter)
    $visibleCount = $task->subtasks->count();
    // Total sub-task count (unfiltered) — passed from TaskController
    $totalCount = $totalSubTaskCount ?? $visibleCount;
@endphp

{{-- Rule 8: Show access restriction notice if some sub-tasks are hidden --}}
@if($totalCount > $visibleCount)
    <div class="alert alert-info d-flex align-items-center mb-3 f-13" role="alert">
        <i class="fa fa-lock mr-2"></i>
        {{ __('Some sub-tasks are hidden due to access restrictions.') }}
    </div>
@endif
